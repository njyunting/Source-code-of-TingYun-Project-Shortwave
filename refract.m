...
total_secrefract = total_refract;
...
for i = 1:size(total_secrefract,1)
    ...
    refractcal = [refractcal;total_secrefract(i,6),dot1,total_secrefract(i,4),dot2,sin];
end
...
% tri_energy = refract_conifer;


load('rgb1904.mat');

for i = 1:size(tri_energy,1)
    pattri = patch(total_tri(tri_energy(i,1),1:3),total_tri(tri_energy(i,1),4:6),total_tri(tri_energy(i,1),7:9),rgb(length(rgb)+1-tri_energy(i,3),:));
    set(pattri,'EdgeColor',rgb(length(rgb)+1-tri_energy(i,3),:))
end
